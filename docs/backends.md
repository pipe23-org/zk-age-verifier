...
The `backend` key of `[service]` selects the pylongfellow backend the service verifies with
(default `google-cpp`; environment override `ZK_AGE_VERIFIER_SERVICE__BACKEND`). Registry
names: `google-cpp` (google/longfellow-zk), `isrg-rust` (abetterinternet/zk-cred-longfellow).
Both ship in the default pylongfellow install. An unknown or unbuilt name fails startup.

## Verification scope

The backends disagree over whether `DeviceNameSpacesBytes` is a verifier input, and the ZK
...
under either backend means the proof holds for a device that signed an empty namespace map.
